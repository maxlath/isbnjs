...
const considerAltPrefix = (normalizedIsbn, altPrefix, clues) => {
  ...
  if (candidateData != null) {
    const { isbn13h, groupname } = candidateData
    clues.push({ message: 'possible prefix error', candidate: isbn13h, groupname })
  }
}
...
const guessPrefixFromChecksum = (normalizedIsbn, altPrefix, clues) => {
  ...
  if (altPrefixData != null) {
    const { isbn13h, groupname } = altPrefixData
    clues.push({ message: 'checksum hints different prefix', candidate: isbn13h, groupname })
  }
}
